...
- [ ] **標準 Provider 診断欄**
  ...
  - Settings にプラグイン独自の診断画面を追加し（[ADR 8](adr/0008-設定画面は読み取り専用の診断に限定しホスト状態を専用rpcで返す.md)）、Paseo 0.9.0 でサーバー設定取得 API を利用したカスタムパス設定・外部リンク導線・マルチホスト表示を導入しました（[ADR 10](adr/0010-paseo-0.9.0のサーバー側設定取得apiでzcodeパスの画面設定を許可する.md)）。検出パス・バージョン・互換性・動作確認済み artifact との一致・保存先を表示し、host check を実行できます。標準診断欄の代替ではなく、本体 API が追加された場合の接続は未実装です。
